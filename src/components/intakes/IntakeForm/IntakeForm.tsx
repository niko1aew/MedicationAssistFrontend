...
import { toDateTimeLocalValue, getCurrentDateTimeLocal } from '../../../utils/formatDate';
...
export const IntakeForm: React.FC<IntakeFormProps> = observer(({
  ...
  const [medicationId, setMedicationId] = useState(intake?.medicationId || preselectedMedicationId || '');
  const [intakeTime, setIntakeTime] = useState(
    intake ? toDateTimeLocalValue(intake.intakeTime) : getCurrentDateTimeLocal()
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Both create and update use same structure for simplicity
    await onSubmit({
      medicationId: intake?.medicationId || medicationId,
      intakeTime: intakeTime ? new Date(intakeTime).toISOString() : undefined,
